...
public class Game {
	...
	/** < @brief Wird linke Maustaste gedrueckt? */
	private static boolean gravRight = false;
	/** < @brief Wird rechte Maustaste gedrueckt? */

	private static Level l;

	/** < @brief Die aktuelle Instanz der Levelklasse */
	 ...
	 * @param a
	 *            FloatRect a muss erheblich kleiner als b sein
	 * @param b
	 *            Das groessere Floatrect
	...
	public static boolean intersection(FloatRect a, FloatRect b) {
		...
		// dadurch muss bei einer Ueberschneidung immer min. ein Eckpunkt im
		// anderen Rechteck liegen
	...
	/**
	 * @brief Ueberpruefen ob im naechsten Schritt zwei SpaceObjects kollidieren
	 *        werden
	 * @param A
	 *            erstes SpaceObject
	 * @param B
	 ...
	 * @param spaceObjects
	 *            liste der Spaceobjects, um alle ueberpruefen zu kuennen
	 */
	public static void schneiden(Vector<SpaceObject> spaceObjects) {
		...
		if (spaceObjects.size() != 0) {
			for (int i = 0; i + 1 <= spaceObjects.size(); i++) {
				...
				for (int j = i + 1; j < spaceObjects.size(); j++) // nur wenn
																	// ein
																	// zweites
																	// Objekt
																	// vorhanden
																	// ist
				{
					...
					// Ist der Abstand der beiden Zentren kleiner als die Summe
					// der Radien?
					if (absVec(Vector2f.sub(P1, P2)) <= Math.abs(A.model.getRadius()) + Math.abs(B.model.getRadius())
							&& !A.elastisch && !B.elastisch) {
						/*
						 * A.getSprite().move(Vector2f.sub(A.model.getVelocity()
						 * ,Vector2f.mul(A.model.getVelocity(), 2)));
						 * B.getSprite().move(Vector2f.sub(B.model.getVelocity()
						 * ,Vector2f.mul(B.model.getVelocity(), 2))); float
						 * Abstand =
						 * (A.model.getRadius()+B.model.getRadius())-absVec(
						 * Vector2f.sub(P1,P2)); float cosPhi =
						 * skalar(A.model.getVelocity(),B.model.getVelocity())/(
						 * absVec(A.model.getVelocity())+absVec(B.model.
						 * getVelocity())); float strecke1a = Abstand * cosPhi;
						 */
						Vector2f x0 = Vector2f.div(Vector2f.sub(P1, P2), absVec(Vector2f.sub(P1, P2))); // Nullvektor
																										// rechtwinklig
																										// zur
																										// Schnittachse
						Vector2f y0 = new Vector2f(-x0.y, x0.x); // Nullvektor
																	// parallel
																	// zu
																	// Schnittachse

						// Berechnung der Energievektorkomponenten
						Vector2f Ax = Vector2f.mul(x0, skalar(A.model.getEnergy(), x0)); // Vektor
																							// A
																							// auf
																							// x0
																							// projiziert
						Vector2f Ay = Vector2f.mul(y0, skalar(A.model.getEnergy(), y0)); // Vector
																							// A
																							// auf
																							// y0
																							// projiziert

						Vector2f Bx = Vector2f.mul(x0, skalar(B.model.getEnergy(), x0)); // Vektor
																							// B
																							// auf
																							// x0
																							// porjiziert
						Vector2f By = Vector2f.mul(y0, skalar(B.model.getEnergy(), y0)); // Vektor
																							// B
																							// auf
																							// y0
																							// projiziert
	 ...
	 * @param args
	 *            Konsolenargumente, die dem Programm uebergeben werden. (Werden
	 *            nicht ausgewertet)
	 */
	public static void main(String args[]) throws InterruptedException, IOException {
		...
		boolean gameOver = false; /**
									 * < @brief true, wenn der Spieler das Spiel
									 * verloren hat
									 */
		boolean youWon = false; /**
								 * < @breif true, wenn der Spieler das Spiel
								 * gewonnen hat
								 */
		int userGravityId = -1; /**
								 * < @brief -1, wenn kein Gravitationszentrum
								 * gesetzt ist
								 */
		...
		while (hauptfenster.isOpen()) {
			...
			for (org.jsfml.window.event.Event ev : hauptfenster.pollEvents()) {
				...
				if (menuAktiv && ev.type == Type.MOUSE_BUTTON_RELEASED) {
					...
					if (spielStartenButton.contains(mousePos)) {
						...
						for (int i = 0; i < levelOptions.size(); i++) {
							if (levelOptions.get(i).getGlobalBounds()
									.contains(hauptfenster.mapPixelToCoords(mousePos))) {
								System.out.println("Lade Level '" + availableLevels.get(i) + "'");

								l = new RandomLevel(availableLevels.get(i));
								menuAktiv = false;
				...
				if (ev.type == Type.MOUSE_BUTTON_PRESSED && ev.asMouseButtonEvent().button == Mouse.Button.LEFT) {
					if (gravLeft == false) {
						gravLeft = true;
						l.gravityFields.addElement(new Gravity((hauptfenster.mapPixelToCoords(
			...
			if (!gameOver && !menuAktiv && !youWon) {
				...
				for (SpaceObject s : l.spaceObjects) {
					...
					if (s.model.isGravityOn()) {
						...
						if (l.blackHoles.size() != 0) { // Wenn Schwarze Loecher
														// enthalten sind,
														// Abstand ueberpruefen
				...
				/**
				 * Abschnitt zur Berechnung der Position der SpaceObjects im
				 * Feld Zuerst Berechnung der umliegenden Sprites und KOllision
				 * Danach Ermittlung auf welchem Tile es momentan ist
				 */
				if (l.spaceObjects.size() != 0) { // es muss mindestens ein
													// Objekt haben
					for (SpaceObject s : l.spaceObjects) {
						...
						for (int f = 1; f < s.Bereich.length; f++) {
							...
							if (tile.index == 1) {
								...
								if (intersection(s.sprite.getGlobalBounds(), FR)) // kollisionsueberpruefung
								{
									...
									boolean Richtung = false; // true oben-unten
																// | false
																// links-rechts
				...
				/**
				 * ueberpruefen der Kollision unter den Objekten Bewegen aller
				 * Objekte
				 */
				...
				if (intersection(l.sprites[1].getGlobalBounds(),
						l.spaceObjects.elementAt(0).sprite.getGlobalBounds())) {
					youWon = true;
				}
				...
				if (moving) {
					if (aktTile2 != l.Feld[(int) l.spaceObjects.get(0).Bereich[0].x][(int) l.spaceObjects
							.get(0).Bereich[0].y]) {
						view.move(l.spaceObjects.get(0).model.getVelocity());
					}
				} else {
					view.setCenter(l.Feld[(int) l.spaceObjects.get(0).Bereich[0].x][(int) l.spaceObjects
							.get(0).Bereich[0].y].center);
				}
				hauptfenster.setView(view);
				...
				if (youWon) {
					youWonSprite.setPosition(hauptfenster.mapPixelToCoords(
							new Vector2i(hauptfenster.getSize().x / 2, hauptfenster.getSize().y / 2)));
				...
				if (menuAktiv) {
					...
					menuSprite.setPosition(hauptfenster.mapPixelToCoords(
							new Vector2i(hauptfenster.getSize().x / 2, hauptfenster.getSize().y / 2)));
